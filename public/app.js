...
socket.on('engineGameCreated', ({ game })=>{
  ...
  if (game.turn !== game.playerColor) setTimeout(()=> requestEngineMove(game.fen, game.difficulty), 500);
  if (currentMode!=='bet') requestEvaluation(game.fen);
});

socket.on('engineTurn', ({ fen })=> requestEngineMove(fen, currentGame?.difficulty));
...
function initStockfish(){
  try{
    ...
    else if (typeof Worker!=='undefined'){ stockfish=new Worker('/stockfish.js'); }
  } catch(e){ console.log('SF error',e); }

  if (!stockfish){ document.getElementById('engineStatus').textContent='Fallback Mode'; engineReady=false; return; }

  stockfish.onmessage = (e)=>{
    const line = e.data || e;
    handleEngineMessage(typeof line==='string'?line:line.data||'');
  };
  if (stockfish.addEventListener) stockfish.addEventListener('message', (e)=> handleEngineMessage(e.data||e));
...
function handleEngineMessage(line){
  ...
  if (line.includes('uciok') || line.includes('readyok')){
    ...
    if (el){ el.textContent='Stockfish Ready ✓'; el.className='status-badge status-playing'; }
  }
  if (line.startsWith('bestmove')){
    const parts=line.split(' ');
    const best=parts[1];
    if (best && best!=='(none)' && pendingEngineResolve){
      const from=best.substring(0,2); const to=best.substring(2,4); const promo=best.length>4?best[4]:undefined;
      pendingEngineResolve({ from,to, promotion:promo, raw:best });
      pendingEngineResolve=null;
      hintMove={ from,to };
    }
    engineThinking=false;
    const thinkEl=document.getElementById('engineThinking');
  ...
  if (line.includes(' score ')){
    ...
    let score=currentEval.score; let text=currentEval.text; let mateVal=null;
    if (cp){ score=parseInt(cp[1])/100; text=(score>0?'+':'')+score.toFixed(1)+(score>0?' White better': score<0?' Black better':' Equal'); }
    if (mate){ mateVal=parseInt(mate[1]); text=`Mate in ${Math.abs(mateVal)}`; score=mateVal>0?10:-10; }
    currentEval={ score, text, depth: depth?depth[1]:'?', pv: pv?pv[1]:'', mate: mateVal };
    updateEvalUI();
    if (pv && currentMode!=='bet'){
      const pvShort=pv[1].split(' ').slice(0,6).join(' ');
      const al=document.getElementById('analysisLines');
      if (al) al.innerHTML=`<div><b>Depth ${currentEval.depth}</b> • ${currentEval.text}</div><div style="margin-top:6px;color:var(--text)">Line: ${pvShort}</div>`;
    }
  }
}
...
function requestEngineMove(fen, difficulty){
  if (!stockfish || !engineReady){ fallbackEngineMove(fen); return; }
  engineThinking=true;
  const thinkEl=document.getElementById('engineThinking'); if (thinkEl) { thinkEl.style.display='flex'; thinkEl.querySelector('#thinkingText').textContent=`Stockfish ${difficultyConfig[difficulty]?.label||''} thinking...`; }
  const cfg=difficultyConfig[difficulty]||{ skill:10, depth:12 };
  const skill=cfg.skill||10;
  const depth=cfg.depth||12;
  const movetime= difficulty==='easy'?300 : difficulty==='medium'?600 : difficulty==='hard'?1000 : 1500;
  stockfish.postMessage(`setoption name Skill Level value ${skill}`);
  stockfish.postMessage(`position fen ${fen}`);
  stockfish.postMessage(`go movetime ${movetime} depth ${depth}`);
  return new Promise((resolve)=>{
    pendingEngineResolve=async (move)=>{
      resolve(move);
      if (currentGame && currentGame.type==='engine'){
        socket.emit('engineReply', { userId, gameId: currentGame.id, from: move.from, to: move.to, promotion: move.promotion }, ()=>{});
      }
    };
    setTimeout(()=>{ if (pendingEngineResolve){ pendingEngineResolve=null; fallbackEngineMove(fen); } }, movetime+2500);
  });
}
function fallbackEngineMove(fen){
  const temp=new Chess(fen); const moves=temp.moves({ verbose:true }); if (!moves.length) return;
  const m=moves[Math.floor(Math.random()*moves.length)];
  if (currentGame && currentGame.type==='engine') socket.emit('engineReply', { userId, gameId: currentGame.id, from: m.from, to: m.to, promotion: m.promotion||'q' });
  const thinkEl=document.getElementById('engineThinking'); if (thinkEl) thinkEl.style.display='none';
}
function requestEvaluation(fen){
  if (!stockfish || !engineReady) return;
  stockfish.postMessage(`position fen ${fen}`);
  stockfish.postMessage(`go depth 14 movetime 800`);
}
...
function startLLMGame(isFree){
  ...
  const color=document.getElementById('llmColor').value==='w'?'w':'b' : 'w';
...
function getPuzzleHint(){
  if (!currentPuzzle) return;
  toast(`Hint: First move ${currentPuzzle.solution[0].toUpperCase()}`, 'info');
  hintMove={ from: currentPuzzle.solution[0].substring(0,2), to: currentPuzzle.solution[0].substring(2,4) };
  renderBoard();
}
...
function getHint(){
  ...
  if (!stockfish || !engineReady) return toast('Engine loading...','info');
  toast('Asking Stockfish for best move...','info');
  stockfish.postMessage(`position fen ${chess.fen()}`);
  stockfish.postMessage(`go depth 16 movetime 1000`);
  const orig=pendingEngineResolve;
  pendingEngineResolve=(move)=>{
    hintMove=move; renderBoard(); toast(`Hint: ${move.from.toUpperCase()} → ${move.to.toUpperCase()}`,'success');
    pendingEngineResolve=orig;
  };
}
function undoMove(){
  if (!currentGame || chess.history().length<2) return toast('Nothing to undo','error');
  if (!currentGame.isFree && currentGame.type!=='pvp_bet') return toast('Undo not allowed in bet mode','error');
  chess.undo(); chess.undo();
  currentGame.fen=chess.fen();
  renderBoard(); renderMovesLocal();
}
...
function doDeposit(){
  ...
  if (!phone||phone.length<9) return toast('EcoCash number','error');
  const btn=document.getElementById('depositBtn'); btn.textContent='⏳...'; btn.disabled=true;
  socket.emit('deposit',{userId,amount,phone}, (res)=>{
    if (res&&res.error){ toast(res.error,'error'); btn.textContent='Deposit'; btn.disabled=false; return; }
    toast(`EcoCash prompt to ${phone} $${amount}`, 'success');
    setTimeout(()=>{ btn.textContent='Deposit'; btn.disabled=false; closeModal('depositModal'); toast(`+$${amount.toFixed(2)} wallet!`,'success'); },2000);
  });
}
function selectWithdrawMethod(el){
  document.querySelectorAll('.method-card').forEach(c=>c.classList.remove('active')); el.classList.add('active'); window.withdrawMethod=el.dataset.method;
  document.getElementById('ecoWithdrawForm').style.display=window.withdrawMethod==='EcoCash'?'block':'none';
  document.getElementById('bankWithdrawForm').style.display=window.withdrawMethod==='Bank'?'block':'none';
}
function doWithdraw(){
  const amount=parseFloat(document.getElementById('withdrawAmount').value);
  if (isNaN(amount)||amount<1) return toast('Min $1','error');
  if (user.balance<amount) return toast('No funds','error');
  let method=window.withdrawMethod||'EcoCash'; let details='';
  if (method==='EcoCash'){ const p=document.getElementById('withdrawPhone').value.trim(); if(!p) return toast('Enter number','error'); details=p; }
  else { const bank=document.getElementById('bankName').value; const acc=document.getElementById('bankAccount').value.trim(); if(!acc) return toast('Enter acc','error'); details=`${bank}-${acc}`; }
  socket.emit('withdraw',{userId,amount,method,accountDetails:details}, (res)=>{ if (res&&res.error) return toast(res.error,'error'); toast(`Withdraw $${amount} to ${method} initiated`,'success'); closeModal('withdrawModal'); });
}
...
function showSolution(){ if (!currentGame) return; toast(`Best line: ${currentEval.pv||'Calculating...'}`, 'info'); }
...
async function checkEcoCashStatus(){
  try{
    const res=await fetch('/api/ecocash/status');
    const data=await res.json();
    const badge=document.getElementById('ecoModeBadge');
    if (badge) badge.textContent = data.mock ? 'MOCK MODE - Auto completes' : 'LIVE MODE - Real API';
    const ecoPill=document.getElementById('jackpotPill');
    // Optionally add eco status
  } catch(e){}
}
setTimeout(checkEcoCashStatus, 1000);
